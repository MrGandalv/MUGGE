...
def write_feature_file(features_file_name, dataset_path, genrelist, featurelist, max_songs_per_genre, overwrite):
    ...
    ch, cha = 0, 0
    headline = "filename"
    if "chord_max" in featurelist:
        ch = 1
        for i in range(1, 145):
            headline += f" chord_transition_matrix{i}"
    if "chord_all" in featurelist:
        cha = 1
        for i in range(1, 145):
            headline += f" chord_all_transition_matrix{i}"
    headline += " label"
    assert max(ch, cha) > 0, "There are no supported features given in 'featurelist'."
    if overwrite == "Append":
        ...
        for filename in os.listdir(f"{dataset_path}/{genre}"):
            for part in range(10):
                ...
                y, sr = librosa.load(songname, mono=True,
                                     duration=3,
                                     offset=3 * part)  # choose with duration , how much is loaded (in seconds)
                line_to_add = f"{part}.{filename}"
                if ch == 1:
                    # Code für die Transition Matrix
                    chromagram = librosa.feature.chroma_stft(y=y, sr=sr)
                    chromagram_columns_number = len(chromagram[0])
                    chromagram_y = np.transpose(chromagram)
                    chromagram_x = np.zeros_like(chromagram_y)
                    chromagram_x[np.arange(len(chromagram_y)), chromagram_y.argmax(1)] = 1
                    chromagram_x_korr = np.transpose(chromagram_x)
                    transition_matrix_a = np.zeros((12, 12))
                    for j in range(chromagram_columns_number - 1):
                        transition_matrix_b = chromagram_x_korr[:, j] * np.transpose([chromagram_x_korr[:, j + 1]])
                    ...
                    for n in range(12):
                        for m in range(12):
                            line_to_add += f" {round(transition_matrix[n][m],6)}"
                if cha == 1:
                    # Code für die Transition Matrix_all
                    chromagram = librosa.feature.chroma_stft(y=y, sr=sr)
                    chromagram_columns_number = len(chromagram[0])
                    transition_matrix_a = np.zeros((12, 12))
                    for j in range(chromagram_columns_number - 1):
...
if __name__ == '__main__':
    features_file_name = "10k_chord_feature.csv"
    ...
    featurelist = "chord_max chord_all".split()
    ...
    #
    write_feature_file(features_file_name, my_dataset_path, genrelist, featurelist, 1000, "")
    #
