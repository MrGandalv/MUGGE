...
def find_best_param(file_name, cv, X, y):
    ...
    # parameter_space = {"kernel": ["linear", "poly", "rbf"], "gamma": ["scale", "auto"], "degree": [3, 5]}
    # parameter_space = {"kernel": ["linear"]}
    parameter_space = {"kernel": ["linear", "poly", "rbf"], "gamma": ["scale", "auto"], "C": [1.0, 0.5]}

    svmclassifier = svm.SVC(random_state=42)
    clf = GridSearchCV(svmclassifier, parameter_space, cv=cv, error_score=0)
    print("step")
    clf.fit(X, y)
...
def find_the_best():
    ...
    for X, name in data_list:
        find_best_param(f"parameter_search_svm_{name}_10k.csv", cv, X, y)
